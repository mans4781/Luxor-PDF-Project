...
export default function PrivacyPage() {
  const toc = useMemo(() => {
    ...
    for (const line of privacyPolicyMarkdown.split("\n")) {
          ...
          <div className="container mx-auto max-w-4xl relative z-10 text-center">
            ...
            >
              ...
              <p className="text-lg lg:text-xl text-slate-600 max-w-2xl mx-auto leading-relaxed">
                How Fairnova Labs Private Limited — owner and operator of the Luxor
                PDF brand — collects, uses, and protects your information.
              </p>
              <div className="mt-8 flex flex-wrap items-center justify-center gap-x-6 gap-y-2 text-sm font-medium text-slate-500">
                <span>
                  Effective&nbsp;
                  <span className="text-slate-800">{EFFECTIVE_DATE}</span>
                </span>
                <span className="hidden h-1 w-1 rounded-full bg-slate-300 sm:inline-block" />
                <span>
                  Last updated&nbsp;
                  <span className="text-slate-800">{LAST_UPDATED}</span>
                </span>
              </div>
            </motion.div>
          </div>
