...
/**
 * Marketing-style page shell for pdf-expiry pages that should feel like part
 * of the luxorpdf.com marketing site (logo + full nav header, marketing
 * footer) rather than the Secure app chrome.
 *
 * The marketing site lives in a different artifact, so its Navbar/Footer can't
 * be imported here. This is a self-contained mirror of that look. All links to
 * marketing routes use plain <a> with absolute root paths so they escape this
 * artifact's `/pdf-expiry` base and hit the marketing app through the shared
 * proxy.
 */

type LinkItem = { label: string; href: string };

const NAV_LINKS: { label: string; href: string; icon: typeof Home }[] = [
...
type SocialLink = { label: string; href: string; color: string; path: ReactNode };
...
const linkIconCls =
  "w-4 h-4 text-[#312E81]/70 group-hover:text-[#DC2626] transition-colors";

const colLinkCls =
  "block text-left text-sm font-semibold text-[#312E81] transition-colors hover:text-[#1E1B4B] rounded-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#E11D48] focus-visible:ring-offset-2 focus-visible:ring-offset-slate-50";
            ...
            <div key={col.title}>
              ...
              <ul className="mt-5 space-y-3">
                {col.links.map(({ label, href }) => (
                  <li key={label}>
                    <a href={href} className={colLinkCls}>
                      {label}
                    </a>
                  </li>
                ))}
...
export function MarketingLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50 text-slate-900">
      {/* Marketing-style header (logo + nav), matching luxorpdf.com */}
        ...
        <div className="container mx-auto px-6 h-20 flex items-center justify-between gap-4">
          ...
          <nav className="hidden lg:flex items-center gap-6">
            {NAV_LINKS.map(({ label, href, icon: Icon }) => (
              <a key={label} href={href} className={`group ${linkCls}`}>
                <Icon className={linkIconCls} strokeWidth={2.2} />
                {label}
              </a>
            ))}
          </nav>
          ...
          <div className="flex items-center gap-2">
            <a
              ...
              className="text-[15px] font-semibold text-[#312E81] hover:text-[#1E1B4B] hover:bg-slate-100 hidden sm:inline-flex items-center rounded-md px-4 py-2 transition-colors"
            >
              Sign in
            </a>
            <a
              href={`${basePath}/sign-up`}
              className="text-[15px] font-semibold bg-[#312E81] hover:bg-[#3730A3] text-white shadow-md shadow-[#312E81]/20 hover:shadow-lg hover:shadow-[#312E81]/30 hover:-translate-y-0.5 rounded-lg hidden sm:inline-flex items-center px-4 py-2 transition-all duration-200"
            >
              Start free →
            </a>
          </div>
