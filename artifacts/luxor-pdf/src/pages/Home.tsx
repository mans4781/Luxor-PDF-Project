...
const Ic = {
  ...
  /* PDF file icon: document sheet with a thin light-blue border and the
     top-right corner clipped into a folded "dog-ear". */
  file: (s = 15) => (
    <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="#8ab8e8" strokeWidth={1.4} strokeLinecap="round" strokeLinejoin="round">
      <path d="M6 3h8l4 4v13a1.5 1.5 0 0 1-1.5 1.5h-9A1.5 1.5 0 0 1 6 20V3z" />
      <path d="M14 3v4h4" />
    </svg>
  ),
        ...
        {recents.length === 0 ? (
          ...
          <>
            {recents.slice(0, 6).map((r) => (
              ...
              >
                <span style={{ flexShrink: 0, display: "flex" }}>{Ic.file(14)}</span>
            ...
            {recents.length === 0 ? (
              ...
              recents.slice(0, 4).map((r) => (
                <button
                  ...
                  onClick={openPicker}
                >
                  <span style={{ flexShrink: 0, display: "flex" }}>{Ic.file(15)}</span>
                  <span style={{ flex: 1, minWidth: 0 }}>
