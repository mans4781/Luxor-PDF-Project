...
export function LinearMinimal() {
  return (
    <div style={{ fontFamily: SANS, background: COLORS.bg, color: COLORS.text, minHeight: "100vh", letterSpacing: "-0.011em" }}>
      {/* NAV */}
      <nav style={{ borderBottom: `1px solid ${COLORS.border}`, background: "white", position: "sticky", top: 0, zIndex: 10 }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "14px 32px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 40 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <div style={{ width: 22, height: 22, background: COLORS.primary, display: "grid", placeItems: "center" }}>
                <FileText size={13} color="white" strokeWidth={2.5} />
              </div>
              <span style={{ fontWeight: 600, fontSize: 15, letterSpacing: "-0.02em" }}>Luxor</span>
            </div>
      ...
      <section style={{ padding: "112px 32px 80px", maxWidth: 1100, margin: "0 auto", textAlign: "center" }}>
        <div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "5px 12px", border: `1px solid ${COLORS.border}`, borderRadius: 999, fontSize: 12, color: COLORS.muted, fontWeight: 500, marginBottom: 32 }}>
          <span style={{ width: 5, height: 5, borderRadius: 999, background: COLORS.accent }} /> Reader 2.0 · Now shipping
        </div>
        <h1 style={{ fontSize: 72, lineHeight: 1.0, fontWeight: 600, letterSpacing: "-0.045em", margin: "0 0 28px" }}>
          PDF, perfected.<br/>
          <span style={{ color: COLORS.muted, fontWeight: 600 }}>One subscription.</span>
        </h1>
        ...
        <div style={{ display: "flex", gap: 10, justifyContent: "center", marginBottom: 72 }}>
          ...
          <button style={{ background: "white", color: COLORS.text, border: `1px solid ${COLORS.borderStrong}`, padding: "11px 20px", borderRadius: 8, fontWeight: 500, fontSize: 14, fontFamily: SANS, cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}>
            See pricing <ArrowUpRight size={14} />
          </button>
        </div>

        {/* APP PREVIEW */}
        <div style={{ borderRadius: 12, border: `1px solid ${COLORS.border}`, overflow: "hidden", background: COLORS.surface, maxWidth: 960, margin: "0 auto" }}>
          <div style={{ background: "white", padding: "10px 16px", borderBottom: `1px solid ${COLORS.border}`, display: "flex", alignItems: "center", gap: 6 }}>
          ...
          <div style={{ height: 280, background: COLORS.surface, display: "grid", placeItems: "center", color: COLORS.muted, fontSize: 13, fontFamily: "ui-monospace, monospace" }}>
      ...
      <section style={{ padding: "32px 32px 80px", maxWidth: 1000, margin: "0 auto", textAlign: "center" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", gap: 32, opacity: 0.6, fontWeight: 600, fontSize: 16, color: COLORS.text, letterSpacing: "-0.02em" }}>
      ...
      <section style={{ padding: "72px 32px 96px", maxWidth: 1200, margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: 56 }}>
          <h2 style={{ fontSize: 48, fontWeight: 600, letterSpacing: "-0.04em", margin: "0 0 12px" }}>
            Simple, fair pricing.
          </h2>
          <p style={{ fontSize: 16, color: COLORS.muted, margin: 0 }}>Start free. Upgrade only when you need to.</p>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 0, border: `1px solid ${COLORS.border}`, borderRadius: 12, overflow: "hidden" }}>
...
function PricingCol({ tier, price, cadence, features, popular }: { tier: string; price: string; cadence: string; features: string[]; popular?: boolean }) {
  return (
    <div style={{
      padding: 32,
      background: popular ? "#FAFAFA" : "white",
      borderRight: `1px solid ${COLORS.border}`,
      position: "relative",
    }}>
      {popular && (
        <div style={{ position: "absolute", top: 16, right: 16, fontSize: 11, color: COLORS.accent, fontWeight: 600, letterSpacing: "0.04em" }}>
          POPULAR
        </div>
      )}
      <div style={{ fontSize: 14, fontWeight: 600, color: COLORS.text, marginBottom: 12 }}>{tier}</div>
      <div style={{ display: "flex", alignItems: "baseline", gap: 4, marginBottom: 4 }}>
        <span style={{ fontSize: 40, fontWeight: 600, letterSpacing: "-0.04em" }}>{price}</span>
      </div>
      ...
      <ul style={{ listStyle: "none", padding: 0, margin: 0, display: "flex", flexDirection: "column", gap: 10 }}>
        {features.map((f) => (
          <li key={f} style={{ display: "flex", gap: 10, alignItems: "center", fontSize: 13, color: COLORS.text }}>
            <Check size={14} color={COLORS.text} strokeWidth={2.5} /> {f}
          </li>
        ))}
      </ul>
    </div>
