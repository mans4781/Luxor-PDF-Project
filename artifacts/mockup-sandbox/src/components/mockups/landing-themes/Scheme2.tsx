    ...
    <div style={{ fontFamily: SANS, background: COLORS.bg, color: COLORS.text, minHeight: "100vh", letterSpacing: "-0.011em" }}>
      ...
      <section style={{ padding: "112px 32px 80px", maxWidth: 1100, margin: "0 auto", textAlign: "center" }}>
        ...
        <div style={{ borderRadius: 12, border: `1px solid ${COLORS.border}`, overflow: "hidden", background: COLORS.surface, maxWidth: 960, margin: "0 auto" }}>
          ...
          <div style={{ height: 280, background: COLORS.bg, display: "grid", placeItems: "center", color: COLORS.muted, fontSize: 13, fontFamily: "ui-monospace, monospace" }}>
...
function PricingCol({ tier, price, cadence, features, popular }: { tier: string; price: string; cadence: string; features: string[]; popular?: boolean }) {
  return (
    <div style={{
      ...
      {popular && (
        <div style={{ position: "absolute", top: 16, right: 16, fontSize: 10, color: "white", background: COLORS.accent, padding: "3px 8px", borderRadius: 999, fontWeight: 700, letterSpacing: "0.06em" }}>
          POPULAR
        </div>
      )}
      ...
      <ul style={{ listStyle: "none", padding: 0, margin: 0, display: "flex", flexDirection: "column", gap: 10 }}>
        {features.map((f) => (
          <li key={f} style={{ display: "flex", gap: 10, alignItems: "center", fontSize: 13, color: COLORS.text }}>
            <Check size={14} color={COLORS.accent} strokeWidth={2.5} /> {f}
          </li>
        ))}
      </ul>
    </div>
