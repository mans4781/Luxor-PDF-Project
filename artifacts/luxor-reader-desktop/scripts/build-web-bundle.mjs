// Builds the luxor-pdf Vite app with a relative base path suitable for
// serving from the app:// protocol inside Electron, then copies the build
// output into `web-bundle/` so electron-builder picks it up via
// `extraResources`.
...
const readerRoot = path.resolve(repoRoot, "artifacts", "luxor-pdf");
const builtDir = path.join(readerRoot, "dist", "public");
...
console.log("[build-web-bundle] building luxor-pdf with BASE_PATH=./");
await run(
  "pnpm",
  ["--filter", "@workspace/luxor-pdf", "run", "build"],
  {
    cwd: repoRoot,
    env: {
      ...process.env,
