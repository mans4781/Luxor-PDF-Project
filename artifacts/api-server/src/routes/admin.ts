...
router.delete(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    const rawUserId = req.params["userId"];
    const userId = typeof rawUserId === "string" ? rawUserId.trim() : "";
    const { rawKey } = req.body as { rawKey?: string };
    if (!userId || typeof rawKey !== "string" || !rawKey.trim()) {
      res.status(400).json({ error: "userId and rawKey are required" });
      return;
    }

    try {
      // The key must hash to an active product key — this proves the value
      // really is the key we just minted (nothing arbitrary gets emailed).
      const { hash } = hashProductKey(rawKey.trim());
      const [keyRow] = await db
        .select()
        .from(productKeysTable)
        .where(
          and(
            eq(productKeysTable.keyHash, hash),
      ...
      if (!recipient) {
        res.status(400).json({ error: "This user has no email address on file" });
        return;
      }
      const name = `${user.firstName ?? ""} ${user.lastName ?? ""}`.trim() || null;

      // Validity date for the email: the user's latest active license end,
      // or (unredeemed key) an estimate from the key's duration.
      const [latest] = await db
        .select({ end: licensesTable.subscriptionEndDate })
        .from(licensesTable)
        .where(
      ...
      const endDate =
        latest?.end ??
        new Date(Date.now() + keyRow.durationDays * 24 * 60 * 60 * 1000);
      ...
      const sent = await sendLicenseEmail({
        to: recipient,
        customerName: name,
        productKey: rawKey.trim(),
        plan: keyRow.planName,
        subscriptionEndDate: endDate.toISOString(),
        downloadUrl,
      });
      if (!sent) {
        res.status(502).json({ error: "Email provider refused the message" });
        return;
      }

      await db.insert(licenseEventsTable).values({
        userId,
        eventType: "license_key_emailed",
        eventMessage: "Reissued license key emailed to customer by admin",
        metadata: {
      ...
      req.log.info(
        { userId, keyPrefix: keyRow.keyPrefix, recipient },
        "Admin emailed reissued license key",
      );
      res.json({ sent: true, recipient });
    } catch (err) {
      req.log.error({ err, userId }, "admin/customers reissue-key/email failed");
      res.status(500).json({ error: "Failed to send the license email" });
    }
  },
);
