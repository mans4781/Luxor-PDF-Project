 ...
 */
export const getGenerateAdScriptUrl = () => {
  return `/api/adstudio/script`;
};

export const generateAdScript = async (
  adScriptInput: AdScriptInput,
  options?: RequestInit,
): Promise<AdScript> => {
  return customFetch<AdScript>(getGenerateAdScriptUrl(), {
    ...options,
    method: "POST",
    headers: { "Content-Type": "application/json", ...options?.headers },
    body: JSON.stringify(adScriptInput),
  });
};

export const getGenerateAdScriptMutationOptions = <
  TError = ErrorType<ErrorResponse>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof generateAdScript>>,
    TError,
    { data: BodyType<AdScriptInput> },
    TContext
  >;
  ...
  TContext
> => {
  const mutationKey = ["generateAdScript"];
  ...
  > = (props) => {
    ...
    return generateAdScript(data, requestOptions);
  };

  return { mutationFn, ...mutationOptions };
};

export type GenerateAdScriptMutationResult = NonNullable<
  Awaited<ReturnType<typeof generateAdScript>>
>;
export type GenerateAdScriptMutationBody = BodyType<AdScriptInput>;
export type GenerateAdScriptMutationError = ErrorType<ErrorResponse>;

/**
 * @summary Generate a marketing video script from product details
 */
export const useGenerateAdScript = <
  TError = ErrorType<ErrorResponse>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
  ...
  TContext
> => {
  return useMutation(getGenerateAdScriptMutationOptions(options));
};
