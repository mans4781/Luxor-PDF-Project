...
- **Sign-in can return `needs_first_factor` even after a correct password** (device/Client-Trust verification, distinct from `needs_second_factor` MFA). Custom sign-in UI must handle BOTH: send `signIn.emailCode.sendCode()` for first factor, `signIn.mfa.sendEmailCode()` for second, and track which factor the code view is verifying. An unhandled status dead-ends with a generic error.
