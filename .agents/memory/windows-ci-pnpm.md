...
When even `git fetch`/`git push` are sandbox-blocked (all local object writes rejected): release entirely via the GitHub Git Data API — diff `git ls-tree -r HEAD` blob shas against the remote tree (`GET /git/trees/<sha>?recursive=1`), upload changed files as blobs, build one tree with `base_tree` = remote head tree, create a single commit on top of remote head, PATCH the branch ref, then create the `reader-v*` tag ref. **Exclude LFS-tracked binaries** (e.g. attached installer .exe files in .gitattributes) — reading them from disk uploads raw bytes, breaking LFS and API size limits. Same divergence side effect as above (contents identical, histories differ). Reader v0.1.9 released this way 2026-07-20.
