...
- Print (toolbar + Ctrl+P) goes STRAIGHT to the browser's native print dialog via a hidden-iframe blob URL — DOM print would show blank virtualized pages. No custom print modal: browsers can't enumerate printers, so an in-app dialog just confuses users. If burn-in edits exist, print builds the edited blob first (same premium gate as export) and fails closed if that build fails (never print unredacted original silently). Ctrl+P uses a latest-ref of handlePrint + a latest-wins token to avoid stale closures/races.
